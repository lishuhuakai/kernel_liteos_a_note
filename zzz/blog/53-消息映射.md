...
LITE_OS_SEC_TEXT STATIC int LiteIpcMmap(struct file *filep, LosVmMapRegion *region)
{
    ...
    //从内核动态空间中申请线性区,分配同等量的物理内存,做好 内核 <-->物理内存的映射
...
LITE_OS_SEC_TEXT STATIC UINT32 LiteIpcWrite(IpcContent *content)
{
    ...
    //这句话是理解上层消息在内核空间数据存放的关键!!! @note_good
...
LITE_OS_SEC_TEXT STATIC UINT32 LiteIpcRead(IpcContent *content)
{
    ...
    do {//注意这里是个死循环
        ...
        if (LOS_ListEmpty(listHead)) {//链表为空 ?
            ...
            //将任务挂到全局链表上,任务进入等IPC信息,等待时间(timeout),此处产生调度,将切换到别的任务执行
...
```
  ...
  * 调度到目标任务后，将切到`LiteIpcRead`执行，此时读函数正在经历一个 `do .. while(1)` 死循环等待消息到来。`LiteIpcRead`的最后是内核地址和用户地址的转换，这也是`LiteIpc`最精彩的部分，它们指向同一块数据。
