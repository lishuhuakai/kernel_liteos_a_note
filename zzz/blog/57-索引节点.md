...
struct Vnode {//vnode并不包含文件名,因为 vnode和文件名是 1:N 的关系
    ...
    //节点引用(链接)数,即有多少文件名指向这个vnode,即上层理解的硬链接数   
