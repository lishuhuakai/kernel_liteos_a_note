用户进程在执行的时候,如果访问到了一个没有被映射的地址,或者对地址执行的操作和地址的属性不符合(例如写了一块只读的内存), 那么 `cpu` 会自动触发 `arm` 的异常处理,具体而言,会跳转到内核 `_osExceptDataAbortHdl` 去执行.
...
写文件的时候发生页面异常,而且写的区域是非共享的线性区,执行 `OsDoCowFault` 来进行写时拷贝,它的思想很简单,那就是分配一块新的内存,将老页面数据拷贝过去,然后将新内存映射到进程的内存空间.
...
读文件的时候发生异常, 会执行 `OsDoReadFault` (加载文件内容,读入内存,然后再做内存映射):
