...
struct socket {
    ...
    void     *s_conn;      /* Connection inherits from struct socket_conn_s */
    ...
    struct devif_callback_s *s_sndcb;
#endif
};

/* Callback from netdev_foreach() */

struct net_driver_s; /* Forward reference. Defined in net/netdev.h */
typedef int (*netdev_callback_t)(struct net_driver_s *dev, void *arg);
...
struct socket *sockfd_socket(int sockfd);
...
int psock_poll(struct socket *psock, struct pollfd *fds, bool setup);
...
int net_clone(struct socket *psock1, struct socket *psock2);
